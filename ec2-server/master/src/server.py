...
def upload_package():
    ...
    if request.method == 'POST':
        ...
        os.system("mkdir -p  data/processing/slave1/input data/processing/slave2/input")

        for ef in files_duo[ : len(files)//2]:
            os.system("mv data/processing/" + ef['file_hash'] + '  data/processing/slave1/input')

        for ef in files_duo[len(files)//2 : ]:
            os.system("mv data/processing/" + ef['file_hash'] + '  data/processing/slave2/input')

        with open('data/processing/slave1/status', 'w') as f:
            f.write('START')
        with open('data/processing/slave2/status', 'w') as f:
            f.write('START')
        with open('data/processing/slave1/package_key', 'w') as f:
            f.write(package_key)
        with open('data/processing/slave2/package_key', 'w') as f:
            f.write(package_key)

        # check if slaves are free
        while True:
            
            time.sleep(3)
            print('checking if slaves are free')

            os.system("aws s3 cp s3://s3-worm/slave1/status data/processing/slave1_status")
            os.system("aws s3 cp s3://s3-worm/slave2/status data/processing/slave2_status")



            with open('data/processing/slave1_status') as f:
                slave1_status = f.read()
                slave1_status = slave1_status.strip()

            with open('data/processing/slave2_status') as f:
                slave2_status = f.read()
                slave2_status = slave2_status.strip()


            if slave1_status == 'STOP' and slave2_status == 'STOP':
                print('slaves are now free...')
                break
            else:
                continue

        print('pushing payload to slaves')
        # push payload to slaves

        os.system("aws s3 sync data/processing/slave1 s3://s3-worm/slave1 --delete")
        os.system("aws s3 sync data/processing/slave2 s3://s3-worm/slave2 --delete")
        ...
        # clean processing dir
        os.system("rm -rf data/processing/*")
        os.system("mkdir -p  data/processing/slave1/ data/processing/slave2/")

        print('retreieving payload from slaves')
        # retrieve data from slaves



        os.system("aws s3 sync s3://s3-worm/slave1/ data/processing/slave1/")
        os.system("aws s3 sync s3://s3-worm/slave2/ data/processing/slave2/")



        # move payload to processing dir
        os.system("mv data/processing/slave1/input/* data/processing/")
        os.system("mv data/processing/slave2/input/* data/processing/")

        # retrieve processing time
        processing_time = 0.0

        with open('data/processing/slave1/processing_time') as f:
            processing_time += float(f.read())

        with open('data/processing/slave2/processing_time') as f:
            processing_time += float(f.read())

        # processing compressed size
        for ef in files_duo:
            compressed_size += os.stat("data/processing/" + ef['file_hash'] + ".enc").st_size
